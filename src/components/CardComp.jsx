...
export default function CardComp({ data }) {
  ...
  return (
    ...
    {
        data.map((item) => (
            <Card key={item.nomor} onClick={() => navigate(`/surat/${data.suratId}`)} className="max-w-sm">
                <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0 text-left">
                            <h3 className="font-semibold text-base sm:text-lg text-foreground transition-colors group-hover:text-primary text-left text-white">
                                {item.namaLatin}
                            </h3>
                            <p className="text-xs sm:text-sm font-medium text-muted-foreground mb-2 text-gray-400">
                                {item.arti}
                            </p>
                        </div>
                    </div>
                    <div>
                        <p className="text-2xl sm:text-3xl text-right text-blue-500">
                            {item.nama}
                        </p>
                    </div>
                </div>
